# Integrase-inhibitor vs efavirenz-based ART in rural Ugandan PMTCT at 18 months
...
- `reproducibility-pack.zip` — the full pack emitted by `student publish --slug art-pmtct-uga`, containing:
  - `paper/` — body + metadata
...
## The body (art-pmtct-uga)

```text
Among HIV-positive pregnant women in Ugandan rural clinics, does integrase-inhibitor ART vs efavirenz-based ART change infant HIV acquisition at 18 months?
We searched PubMed, Embase, and ClinicalTrials.gov through 2026 for randomised or high-quality cohort evidence.
We fit a random-effects model with Paule-Mandel tau-squared and computed an HKSJ odds ratio across 6 studies.
The pooled odds ratio was 0.81 with a 95 percent confidence interval of 0.58 to 1.14, indicating a possible 19% reduction.
...
```
student new --template T0 --slug art-pmtct-uga
# ... draft current_body.txt ...
student validate --path current_body.txt --strict
student baseline record art-pmtct-uga --from report.json
student verify-citations --path current_body.txt
student enroll-authors --slug art-pmtct-uga
student publish --slug art-pmtct-uga
```
